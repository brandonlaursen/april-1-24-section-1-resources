...
- closure
- filter through this array and eliminate the elements that are NOT the datatype
- return a func that takes in a type
- create a new array and populate it with ele from the other func that are not the same type
...
function eliminateType(arr) {
  return function recursion (type, index = 0, result = []) {
    if (index === arr.length) {
      return result;
    }
    ...
    return recursion(type, index + 1, result);
  }
}
