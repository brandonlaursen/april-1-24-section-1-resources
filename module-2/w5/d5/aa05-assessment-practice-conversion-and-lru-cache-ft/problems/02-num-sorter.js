...
class LinkedList {
  ...
  // ! removing from the front -> array.shift()
  dequeue() {
    if (!this.head) {
      return null;
    }
...
class NumSorter {
  constructor() {
    // ! OG CODE
    // this.numList = []; // LinkedList

    // this.allowedNums = []; // Set

    // * Upgrade
      this.numList = new LinkedList(); // LinkedList
  ...
  */
  addNumToBack(num) {
    // ! OG: O(1) best case, O(n) worst case if we need to resize the array -> .push()
    // if (this.isNumAllowed(num)) this.numList.push(num);
    // return this.numList[this.numList.length - 1];

    // * Upgrade: O(1) -> enqueue, return the tail's value
  ...
  */
  ...
  numCount() {
    ...
    // return count;
    // * Upgrade: O(1) -> length property of the LinkedList
  ...
  */
  buildNumList(amount) {
    // ! using an array
    // ! O(n^2) -> for loop, nested .includes
    // this.numList = [];
    ...
    // * Upgrade: O(n) uses linked list, O(1) -> .has, O(1) -> enqueue
